    ...
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          setFormData({ name: '', email: '',phone:'', messages: '' });
        ...
        <>
            <div class="font-poppins max-w-6xl mx-auto relative bg-white rounded-lg py-6">
                <div class="grid lg:grid-cols-3 items-center">
                    ...
                    <div class="lg:col-span-2 bg-purple-500 rounded-lg sm:p-10 p-4 z-10 max-lg:-order-1 max-lg:mb-8">
                        <h2 class="text-3xl text-white text-center font-bold mb-6">Contact us</h2>
                        <form onSubmit={handleSubmit}>
                            <div class="max-w-md mx-auto space-y-3">
                                <input type='text' placeholder='Name' name='name'
                                    class="w-full bg-gray-100 rounded-lg py-3 px-6 text-sm outline-none" value={formData.name} onChange={handleChange} />
                                <input type='email' placeholder='Email' name='email'
                                    class="w-full bg-gray-100 rounded-lg py-3 px-6 text-sm outline-none" value={formData.email} onChange={handleChange}/>
                                <input type='number' placeholder='Phone No.' name='phone'
                                    class="w-full bg-gray-100 rounded-lg py-3 px-6 text-sm outline-none" value={formData.phone} onChange={handleChange}/>
                                <textarea placeholder='Message' rows="6" name='messages'
                                    class="w-full bg-gray-100 rounded-lg px-6 text-sm pt-3 outline-none" value={formData.messages} onChange={handleChange}></textarea>
                                <button type='submit'
                                    class="text-white w-full relative bg-pink-600 hover:bg-pink-700 font-semibold rounded-lg text-sm px-6 py-3 !mt-6">
